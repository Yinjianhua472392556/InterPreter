...
class SemanticAnalyzer: Visitor {
    ...
    func analyze(node: AST) -> [String: ScopedSymbolTable] {
        visit(node: node)
        return scopes
    }
    ...
    func visit(procedure: Procedure) {
        ...
        case let function as Function:
            ...
            let fn = FunctionSymbol(name: procedure.name, params: parameters, body: procedure, returnType: symbol)
            scope.enclosingScope?.insert(fn)
        default:
            let proc = ProcedureSymbol(name: procedure.name, params: parameters, body: procedure)
    ...
    func checkFunction(call: FunctionCall, procedure: ProcedureSymbol) {
